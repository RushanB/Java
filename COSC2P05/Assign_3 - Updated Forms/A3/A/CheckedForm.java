...
public class CheckedForm extends JFrame implements ActionListener, FocusListener{
 ...
 public class Node{ //stores data for each JTextField within CheckedForm
  ...
  public Node(JTextField field, Type kind, String name, Node next){
 ...
 public JTextField addField(String label,Type kind){ //adds a new JTextField to the form
 ...
 private void fieldNode(JTextField field, String name, Type kind){ //adds a field based on JTextField
  if(type == null){
 ...
 private boolean checkForm(String name, Type kind){ //method checks each field, red for incorrect and black for correct
  ...
  if((formatCheck) && (!ifEmpty())){
   ...
   System.out.println("Incorrect input, errors are highlighted in red");
   return false;
  }  
 }
 @Override
  ...
  public void actionPerformed(ActionEvent e){
   ...
   if(e.getSource().equals(accept)){
    ...
    if(thread.getName().compareTo("Thread-5") == 0){
     thread.interrupt();
     System.out.println("Acme Distribution Form is now closed");  //print exit statement to console
    }
   }
  }
 }
